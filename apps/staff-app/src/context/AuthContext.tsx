...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  useEffect(() => {
    const bootstrapAsync = async () => {
      try {
        const storedUser = await AsyncStorage.getItem('staff_user');
        const storedAccess = await getSecureToken('staff_accessToken');
        const storedRefresh = await getSecureToken('staff_refreshToken');
  ...
  const login = async (email: string, password: string) => {
    try {
      ...
      if (data.user.role === Role.CUSTOMER) {
        return {
          success: false,
          error: 'Yetkisiz hesap. Müşteri hesabı ile personel uygulamasına giriş yapılamaz.',
        };
      }
      ...
      await saveSecureToken('staff_accessToken', data.accessToken);
      await saveSecureToken('staff_refreshToken', data.refreshToken);
  ...
  const logout = async () => {
    ...
    await deleteSecureToken('staff_accessToken');
    await deleteSecureToken('staff_refreshToken');
  };
    ...
    if (response.status === 401 && refreshToken) {
      ...
      try {
        ...
        if (refreshResponse.ok) {
          ...
          await AsyncStorage.setItem('staff_user', JSON.stringify(refreshData.user));
          await saveSecureToken('staff_accessToken', refreshData.accessToken);
          await saveSecureToken('staff_refreshToken', refreshData.refreshToken);
