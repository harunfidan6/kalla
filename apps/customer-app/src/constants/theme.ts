...
// Converted from the design handoff's tokens(dark) function (Kalla Demo.dc.html) —
// oklch() has no React Native equivalent, so every value below is a manual hex/rgba
// approximation of the original oklch token, done once here.
...
export const Fonts = {
  ...
  // Upright (non-italic) Newsreader — used for product names, prices/amounts, order totals,
  // notification titles per the design handoff (only screen headings & greetings are italic).
...
// Matches the design handoff's fmtTL(): tr-TR locale, always 2 decimals, comma separator.
